...
if __name__ == "__main__":
    ...
    for episode in range(episodes):
        ...
        for iteration in range(max_iterations):
            ...
            total_reward += reward
            q_next = np.max(Q(W, next_state))

            current_q = Q(W, state, action)
            target = (gamma * q_next) + reward
            W[action] += lr * (target - current_q) * np.concatenate(([1], state))
            state = next_state
    ...
    np.savetxt(returns_out, returns, fmt="%.18e")
