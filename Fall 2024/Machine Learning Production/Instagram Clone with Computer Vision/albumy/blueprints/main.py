...
endpoint = config.get('AZURE_ENDPOINT_URL')
analyze_url = os.path.join(endpoint, "vision/v3.2/analyze")

if not subscription_key or not endpoint:
    raise ValueError("Azure API key or endpoint is missing.")


def generate_alt_text_and_tags(image_path):
    ...
    # Extract alt text and tags from the Azure response
    alt_text = analysis["description"]["captions"][0]["text"] if analysis["description"]["captions"] else "No description available."
...
def search():
    q = request.args.get('q', '').strip()  # Get the search query
    if q == '':
        flash('Enter a keyword about photo, user, or tag.', 'warning')
        return redirect_back()

    category = request.args.get('category', 'photo')  # Check if searching by photo, user, or tag
    page = request.args.get('page', 1, type=int)
    ...
    if category == 'user':
        ...
        # Search for photos that are associated with the given tag
        tag = Tag.query.filter_by(name=q).first()
        if tag:
            # Get all photos associated with the found tag
            pagination = Photo.query.filter(Photo.tags.contains(tag)).paginate(page, per_page)
        else:
            flash(f'No results found for tag "{q}".', 'warning')
            return redirect_back()
    else:
...
def upload():
    if request.method == 'POST' and 'file' in request.files:
        ...
        # Resize the image (existing functionality)
        filename_s = resize_image(f, filename, current_app.config['ALBUMY_PHOTO_SIZE']['small'])
        filename_m = resize_image(f, filename, current_app.config['ALBUMY_PHOTO_SIZE']['medium'])

        # Generate alt text and tags using Azure API
        alt_text, detected_tags = generate_alt_text_and_tags(file_path)

        # Create a new Photo object with alt text
        photo = Photo(
            filename=filename,
            filename_s=filename_s,
            filename_m=filename_m,
            author=current_user._get_current_object(),
            description=alt_text  # Use the alt text as the description
        )

        # Add the detected tags to the Photo
        ...
        # Save the photo and commit everything to the database
